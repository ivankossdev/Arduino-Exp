...
volatile bool takeSample = false; // флаг для одного замера
volatile int sampleIndex = 0;
const unsigned long samplePeriod_us = 1000000.0 / sampleFreq;
...
void IRAM_ATTR onTimer(void *arg) {
  if (sampleIndex < nSamples) {
    takeSample = true; // только ставим флаг
  } else {
    dataReady = true;
  }
}
...
void loop() {
  // Делаем замер в основном цикле, когда таймер поставил флаг
  if (takeSample && sampleIndex < nSamples) {
